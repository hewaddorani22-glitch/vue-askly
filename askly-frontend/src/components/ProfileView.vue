...
const submitBugReport = async () => {
  ...
  try {
    // Bug-Reports werden lokal gesammelt (kein externer Dienst nötig).
    const reports = JSON.parse(localStorage.getItem('askly_bug_reports') || '[]')
    reports.unshift({
      message: bugReportText.value.trim(),
      userName: userName.value,
      timestamp: new Date().toISOString(),
    })
    localStorage.setItem('askly_bug_reports', JSON.stringify(reports))
    bugReportText.value = ''
